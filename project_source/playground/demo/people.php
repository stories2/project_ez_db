<?php
	...
	if(empty($_GET['id']) != true && empty($_GET['page'])!=true)
	{
		...
		$query = "select address,people_num,latitude,longitude from people order by people_num desc;";
...
<body top="0" left="0">
    ...
    <div class="container metro">
		...
		while($row = mysqli_fetch_array($result,MYSQLI_NUM))
		{
			...
			{
				$send_msg = $row[2]."|".$row[3];
				echo '
			                <a class="metro-command-button" onclick="window.external.get_location(\''.$send_msg.'\')">	
		        	            <span class="title">Top'.$cnt.'</span>
                			    <span class="subtitle">'.$row[0].'</span>
			                </a>';
			}
		...
		{	
			echo '
			<a class="metro-button " onclick="window.external.people_prev_page()">앞으로가기</a>';
		}
		...
		{
			echo '
			<a class="metro-button disabled" onclick="" >뒤로 가기</a>';
		}
		else
		{
			echo '
			<a class="metro-button " onclick="window.external.people_next_page()">뒤로 가기</a>';
		}
		echo '
                </left>
        </footer>
